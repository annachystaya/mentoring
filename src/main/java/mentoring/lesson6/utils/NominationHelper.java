package mentoring.lesson6.utils;
...
public class NominationHelper {
    ...
    private void createNominationWithNominatorLimits(Nominator nominator1, Nominee nominee1) {
        ...
        */
        for (int quantity = 1; quantity <= nominator1.getAwardQuantityLimit(); quantity++) {
            nominator1.generateNextAward();
            nominator1.nominate(nominee1);
         ...
         */
        nominator1.generateNextAward();
        while (currentAmount + nominator1.getAward().getValue() < nominator1.getAwardAmountLimit()) {
            nominator1.nominate(nominee1);
    ...
    private void createNominationWithNomineeLimits(Nominator nominator1, Nominee nominee1) {
        ...
        while ((currentAmount + nominator1.getAward().getValue() <= nominee1.getAwardAmountLimit()) && (currentQuantity < nominee1.getAwardQuantityLimit()));

        if (currentAmount + nominator1.getAward().getValue() > nominee1.getAwardAmountLimit()) {
            errorCode = 2;
        }
        if (currentQuantity >= nominee1.getAwardQuantityLimit()) {
            errorCode = 1;
        }
        switch (errorCode) {
            case 1:
